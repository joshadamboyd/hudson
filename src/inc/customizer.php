...
function hudson_customize_register( $wp_customize ) {
	$color_scheme = hudson_get_color_scheme();
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'hudson_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'wp_hudson' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => hudson_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'wp_hudson' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'wp_hudson' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'wp_hudson' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'wp_hudson' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'wp_hudson' );
}
add_action( 'customize_register', 'hudson_customize_register', 11 );

/**
 * Register color schemes for theme.
 *
 * Can be filtered with {@see 'hudson_color_schemes'}.
 *
 ...
 */
function hudson_get_color_schemes() {
	return apply_filters( 'hudson_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'wp_hudson' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'hudson_get_color_scheme' ) ) :
/**
 * Get the current theme color scheme.
 *
 * @return array An associative array of either the current or default color scheme hex values.
 */
function hudson_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = hudson_get_color_schemes();
...
endif; // hudson_get_color_scheme

if ( ! function_exists( 'hudson_get_color_scheme_choices' ) ) :
/**
 * Returns an array of color scheme choices registered for theme.
 *
 * @return array Array of color schemes.
 */
function hudson_get_color_scheme_choices() {
	$color_schemes                = hudson_get_color_schemes();
...
endif; // hudson_get_color_scheme_choices

if ( ! function_exists( 'hudson_sanitize_color_scheme' ) ) :
/**
 ...
 */
function hudson_sanitize_color_scheme( $value ) {
	$color_schemes = hudson_get_color_scheme_choices();
...
endif; // hudson_sanitize_color_scheme
 ...
 */
function hudson_color_scheme_css() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	...
	$color_textcolor_rgb         = hudson_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = hudson_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = hudson_get_color_scheme_css( $colors );

	wp_add_inline_style( 'hudson-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'hudson_color_scheme_css' );
 ...
 */
function hudson_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', hudson_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'hudson_customize_control_js' );

/**
 * Binds JS handlers to make the Customizer preview reload changes asynchronously.
 */
function hudson_customize_preview_js() {
	wp_enqueue_script( 'hudson-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'hudson_customize_preview_js' );
 ...
 */
function hudson_get_color_scheme_css( $colors ) {
 ...
 * preview.
 */
function hudson_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-hudson-color-scheme">
		<?php echo hudson_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'hudson_color_scheme_css_template' );
